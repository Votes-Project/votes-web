...
@live
let makeLink = (~linkBrightID: option<int>=?, ~voteDetails: option<int>=?, ~voteDetailsToken: option<int>=?, ~stats: option<int>=?, ~answer: option<int>=?, ~useVote: option<int>=?, ~question: option<string>=?) => {
  open RelayRouter.Bindings
...
let makeLinkFromQueryParams = (queryParams: queryParams) => {
  makeLink(~linkBrightID=?queryParams.linkBrightID, ~voteDetails=?queryParams.voteDetails, ~voteDetailsToken=?queryParams.voteDetailsToken, ~stats=?queryParams.stats, ~answer=?queryParams.answer, ~useVote=?queryParams.useVote, ~question=?queryParams.question, )
}
